...
- [chafa](https://hpjansson.org/chafa/) only if you want image logos (`--image` / `CETCH_IMAGE`)
...
| | `--image [SRC]` | Draw an image logo instead of ASCII; `SRC` is a path, or `auto` for the built-in distro image (default `auto` if omitted). Needs [chafa](https://hpjansson.org/chafa/) |
...
| `CETCH_IMAGE` | same as `--image` (`auto` for the distro image, or a file path) |
| `CETCH_IMAGE_SIZE` | same as `--image-size` |
| `CETCH_LOGOS_DIR` | directory of built-in distro images (defaults to `DistroLogos/` next to the script, or a packaged share path) |
...
ASCII art is the default. To use a real image instead:

1. Install [chafa](https://hpjansson.org/chafa/).
2. On first run, `cetch` creates `~/.config/cetch/cetch.conf` with the image
   options commented out. Uncomment one of:

   ```
   # built-in image for your detected distro (from DistroLogos/)
   CETCH_IMAGE=auto

   # or any image file you like
   # CETCH_IMAGE=~/Pictures/my-logo.png

   # optional width in terminal cells (default 24)
   # CETCH_IMAGE_SIZE=24
   ```

3. Or pass it on the command line: `cetch --image`, `cetch --image auto`,
   or `cetch --image ~/Pictures/logo.png`.

Built-in images live in `DistroLogos/` in the repo (and are installed under
the package share directory on Nix). If chafa is missing or the image cannot
be read, `cetch` falls back to the ASCII logo and prints a warning.

## Config file

On first run, `cetch` creates `~/.config/cetch/cetch.conf` (if it does not
already exist) with commented examples, including the image options above.
It reads that file before it looks at anything you typed.
